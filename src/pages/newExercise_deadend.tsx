...
export default function NewExercise() {
  ...
  const [temporaryLatex, setTemporaryLatex] = useState("");
  ...
  const operationsByCategory = {
    ...
    placeholder: [{ label: "placeholder", command: "\\placeholder[]{}" }],
  };
  const updateCard = (index, updatedProps) => {
    setCards(
      cards.map((card, i) =>
        i === index ? { ...card, ...updatedProps } : card
      )
    );
  };
    ...
    if (currentCardIndex !== null) {
      ...
      if (inputElement) {
        ...
        if (activeField.type === "alternative") {
          updateAlternative(currentCardIndex, activeField.index, {
            latex: updatedValue,
          });
        } else if (activeField.type === "hint") {
          updateHint(currentCardIndex, activeField.index, {
            latex: updatedValue,
          });
  ...
  const handleCorrectChange = (cardIndex, altIndex) => {
    ...
    updatedCards[cardIndex].alternatives = updatedCards[
      cardIndex
  ...
  const handleSave = () => {
    const updatedCards = cards.map((card, index) => {
      ...
      const successMessageInput = document.querySelector(
        `#card-success-message-${index}`
      );
      ...
      return {
        ...
        ...(card.type === "alternativas" && {
          alternatives: card.alternatives.map((alt, altIndex) => ({
            ...alt,
            text:
              document.querySelector(`#card-alternative-${index}-${altIndex}`)
                ?.value || alt.text,
          })),
        }),
        ...
        ...(card.type === "singleplaceholder" && {
          placeholders:
            document.querySelector(`#card-placeholder-${index}`)?.value ||
            card.placeholders,
          respuestas:
            document.querySelector(`#card-respuestas-${index}`)?.value ||
            card.respuestas,
        }),
        ...
        hints: card.hints.map((hint, hintIndex) => ({
          ...hint,
          text:
            document.querySelector(`#card-hint-${index}-${hintIndex}`)?.value ||
            hint.text,
        })),*/
      };
    });
    ...
    const exerciseTopicSelect = document.querySelector(
      "#exercise-topic-select"
    );
    ...
    // Guarda el JSON en localStorage y redirige
    localStorage.setItem("ejercicioJSON", ejercicioJSON);
  ...
  const handleButtonClick = useCallback((cardIndex, answerIndex) => {
    setCards((prevCards) => {
      ...
      currentRespuestas[answerIndex] =
        currentRespuestas[answerIndex] === "V" ? "F" : "V"; // Toggle value
      return updatedCards;
    });
  }, []);
      ...
      <div>
        ...
        >
          {cards.map((card, index) => (
            ...
            >
              <Text
                ...
                fontWeight="bold"
              >
                {getCardLabel(card.type) === "Enunciado"
              ...
              <Box flex="1" w="100%">
                {card.type != "enunciado" ? (
                  <>
                    <Input
                      ...
                      placeholder={`Pregunta/Texto del paso`}
                      bg="white"
                      mb={2}
                      onBlur={(e) =>
                    ...
                    <Box mb={2}>
                      <Box display="flex" alignItems="center">
                        ...
                        >
                          {card.latex ? (
                            <MathJax>{`\\(${card.expression}\\)`}</MathJax> // Renderiza LaTeX si existe contenido
                          ) : (
                            "Expresión" // Texto fijo "Expresión"
                          )}
                        </Box>
                        {/* Botón para abrir el modal de edición de LaTeX */}
                        <Button
                          size="xs"
                          ml={2}
                          onClick={() => (
                            setCurrentCardIndex(index),
                            setTemporaryLatex(cards[index]?.latex || ""),
                            onOpenLatex()
                          )}
                        >
                          ✏
                ...
                {card.type === "enunciado" ? (
                  <>
                    ...
                    />
                    <Input
                      placeholder={`Expresión del enunciado`}
                      bg="white" //agregar modal de latex + guardar campo
                      mb={2}
                      onBlur={(e) =>
                        updateCard(index, { expression: e.target.value })
                      }
                    />
                  </>
                ) : card.type === "blank" ? (
                    ...
                    >
                      ...
                      <Flex key={index} align="center" gap={2}>
                        ...
                        <Flex align="center" gap={2} flex="1">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setTemporaryLatex(cards[index]?.blanklatex || "")
                        ...
                        <Select
                          ...
                          flex="1"
                        >
                          <option value="StringComparison">
                            StringComparison
                          </option>
                          <option value="EvaluateandCount">
                            EvaluateandCount
                          </option>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setCards((prevCards) =>
                            prevCards.map((c, i) =>
                              i === index
                                ? { ...c, metodoCorreccion: e.target.value }
                                : c
                            )
                          )
                        }
                    ...
                    >
                      ...
                      <Flex align="center" mb={4}>
                        <Button
                          ...
                          onClick={() =>
                            handleTrueFalseChange(index, "trueOption")
                          }
                          mr={2}
                          mt={4}
                        >
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleTrueFalseChange(index, "falseOption")
                          }
                          mt={4}
                        >
                          Falso
                        ...
                        <Flex key={altIndex} align="start" mb={4}>
                            ...
                            <Flex align="center" gap={2}>
                              {alt.isEditing ? (
                                <Input
                                  ...
                                  value={alt.latex || ""}
                                  onChange={(e) =>
                                    updateAlternative(index, altIndex, {
                          ...
                          >
                            ...
                            <Checkbox
                              ...
                              onChange={() =>
                                handleCorrectChange(index, altIndex)
                              }
                              mb={2}
                            >
                              Correcta
              ...
              {card.type !== "enunciado" && (
                <Box flex="1" w="100%">
                  ...
                  >
                            ...
                            {hint.isEditing ? (
                              ...
                              >
                                {hint.latex ? (
                                  ...
                                  <Text color="gray.500" fontSize="sm">
                                    Haz clic en el lapiz para agregar expresión
                                  </Text>
                                )}
                              </Box>
                            )}
                    ...
                    <Button mt={4} onClick={() => addHints(index)}>
                      Agregar pista
                    </Button>
                  </Box>
                  {/* Demas imputs */}
                  ...
                  />
                  <Input
                    id={`card-incorrect-message-${index}`}
                    placeholder="Mensaje de fallo"
                    bg="white"
                    mb={4}
                  />
                  <Input
              ...
              onClick={() => {
                cards.length <= 1
                  ? toast({
                      title: "Error",
                      description:
                        "No puedes guardar sin tener pasos en el ejercicio",
                      status: "error",
                      duration: 5000,
                      isClosable: true,
                    })
              ...
              <ModalBody>
                ...
                >
                  ...
                  <option value="verdadero/falso">
                    Paso de verdadero/falso
                  </option>
                  <option value="blank">Paso de blank</option>
                  <option value="multipleplaceholder">
                    Paso de placeholders
                  </option>
              ...
              <ModalBody>
                <Stack spacing={3}>
                  ...
                  <Flex align="center">
                    ...
                    <Input placeholder="Código Del Ejercicio" />
                  </Flex>
                  <Flex align="center">
                    ...
                    >
                      ...
                      <option value="Productos Notables">
                        Productos Notables
                      </option>
                    </Select>
                  </Flex>
          ...
          <Modal
            isOpen={isLatexOpen}
            onClose={() => {
              // Al cerrar, aplicar cambios de temporaryLatex a cards
              if (currentCardIndex !== null) {
                if (activeAlternativeIndex !== null) {
                  updateAlternative(currentCardIndex, activeAlternativeIndex, {
                    latex: temporaryLatex,
                  });
                } else if (activeHintIndex !== null) {
                  updateHint(currentCardIndex, activeHintIndex, {
                    latex: temporaryLatex,
                  });
                } else if (isBlankActive) {
                  updateCard(currentCardIndex, { blank: temporaryLatex });
                } else if (isPlaceholderActive) {
                  updateCard(currentCardIndex, {
                    placeholders: temporaryLatex,
                  });
                } else if (isResponseActive) {
                  updateCard(currentCardIndex, { respuestas: temporaryLatex });
                } else {
                  updateCard(currentCardIndex, { latex: temporaryLatex });
                }
              }
              ...
              setTemporaryLatex(""); // Limpiar variable temporal
            }}
            size="xl"
          >
            <ModalOverlay />
            <ModalContent>
              ...
              <ModalBody>
                {currentCardIndex !== null && (
                  <>
                    {/* Variable temporal para almacenar el valor de LaTeX */}
                    <Input
                      ...
                      value={temporaryLatex}
                      onChange={(e) => setTemporaryLatex(e.target.value)} // Actualiza solo la variable temporal
                      mb={3}
                    />
                    ...
                    <Tabs variant="line" colorScheme="gray">
                      <TabList>
                        {Object.keys(operationsByCategory).map(
                          (category, index) => (
                            <Tab
                              key={index}
                              _selected={{
                      ...
                      <TabPanels>
                        {Object.values(operationsByCategory).map(
                          (operations, index) => (
                            <TabPanel key={index}>
                              <Flex wrap="wrap" gap={2}>
                                {operations.map(
                                  ({ label, command }, opIndex) => (
                                    <Button
                                      key={opIndex}
                                      size="sm"
                                      variant="outline"
                                      ...
                                      } // Inserta el comando en la variable temporal
                                    >
                                      <MathJax
                                        dynamic
                                        inline
              ...
              <ModalFooter>
                <Button
                  colorScheme="blue"
                  onClick={() => {
                    // Al cerrar el modal, aplicar cambios
                    if (currentCardIndex !== null) {
                      if (activeAlternativeIndex !== null) {
                        updateAlternative(
                          currentCardIndex,
                          activeAlternativeIndex,
                          { alternativalatex: temporaryLatex }
                        );
                      } else if (activeHintIndex !== null) {
                        updateHint(currentCardIndex, activeHintIndex, {
                          hintlatex: temporaryLatex,
                        });
                      } else if (isBlankActive) {
                        updateCard(currentCardIndex, { blanklatex: temporaryLatex });
                      } else if (isPlaceholderActive) {
                        updateCard(currentCardIndex, {
                          placeholderslatex: temporaryLatex,
                        });
                      } else if (isResponseActive) {
                        updateCard(currentCardIndex, {
                          respuestaslatex: temporaryLatex,
                        });
                      } else {
                        updateCard(currentCardIndex, { expression: temporaryLatex });
                      }
                    }
                    onLatexClose(); // Cerrar el modal
                    setActiveAlternativeIndex(null);
                    setActiveHintIndex(null);
                    setIsBlankActive(false);
                    setIsPlaceholderActive(false);
